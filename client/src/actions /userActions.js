...
export const submitKYCForm = (userData) => async (dispatch, getState) => {
  dispatch({ type: USER_KYC_REQUEST, payload: userData });
  const {
    userSignin: { userInfo },
  } = getState();
  try {
    const { data } = await Axios.post(`${serverUrl}/client/kyc`, userData, {});
    dispatch({ type: USER_KYC_SUCCESS, payload: data });
  } catch (error) {
    const message =
      error.response && error.response.data.message
        ? error.response.data.message
        : error.message;
    dispatch({ type: USER_KYC_FAIL, payload: message });
  }
};
